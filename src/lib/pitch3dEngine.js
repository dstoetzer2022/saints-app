...
export function buildScene(THREE, mount, pitcher, opts) {
  ...
  let raf;
  function tick(){raf=requestAnimationFrame(tick);const active=state.built[state.activeIdx];if(!active){renderer.render(scene,camera);return;}
  ...
  state.dispose=()=>{cancelAnimationFrame(raf);mount.removeEventListener("mousedown",onDown);window.removeEventListener("mouseup",onUp);window.removeEventListener("mousemove",onMove);mount.removeEventListener("wheel",onWheel);mount.removeEventListener("touchstart",onTStart);window.removeEventListener("touchend",onUp);window.removeEventListener("touchmove",onTMove);window.removeEventListener("resize",onResize);renderer.dispose();if(mount.contains(renderer.domElement))mount.removeChild(renderer.domElement);};
  return state;
}
